    ...
    try {
        if(name){
            if(id_type){
                ...
                if(typesort == 1){
                    const itemList = await Item.sequelize.query(
                        "SELECT I.*, T.name as name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = I.id_item) as rating FROM items as I, types as T WHERE T.id_type = I.id_type AND I.status != 0 AND I.name COLLATE UTF8_GENERAL_CI LIKE :name ORDER BY rating DESC, I.price ASC LIMIT :from,:perPage", 
                    { 
            ...
            if(id_type){
                ...
                if(typesort == 1){
                    const itemList = await Item.sequelize.query(
                        "SELECT DISTINCT I.*, T.name as name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = I.id_item) as rating FROM items as I, types as T, reviews as R WHERE T.id_type = I.id_type AND I.status != 0 AND T.id_type = :id_type ORDER BY rating DESC, I.price ASC LIMIT :from,:perPage", 
                    { 
                ...
                if(typesort == 1){//rating giam dan
                    const itemList = await Item.sequelize.query(
                        "SELECT I.*, T.name AS name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = I.id_item) as rating FROM items as I, types as T WHERE T.id_type = I.id_type AND I.status != 0 ORDER BY rating DESC, I.price ASC LIMIT :from,:perPage", 
                    { 
    ...
    try {
        if(name){
            if(id_type){
                ...
                if(typesort == 1){
                    const itemList = await Item.sequelize.query(
                        "SELECT I.*, T.name as name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = I.id_item) as rating FROM items as I, types as T WHERE T.id_type = I.id_type AND I.status != 0 AND T.id_type = :id_type AND I.name COLLATE UTF8_GENERAL_CI LIKE :name ORDER BY rating DESC, I.price ASC", 
                    { 
                        replacements: { name: `%${name}%`, id_type: id_type },
                        type: QueryTypes.SELECT,
                        raw: true
                    ...
                    const itemList = await Item.sequelize.query(
                        "SELECT I.*, T.name as name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = I.id_item) as rating FROM items as I, types as T WHERE T.id_type = I.id_type AND I.status != 0 AND T.id_type = :id_type AND I.name COLLATE UTF8_GENERAL_CI LIKE :name ORDER BY I.price DESC", 
                    { 
                        replacements: { name: `%${name}%`, id_type: id_type },
                        type: QueryTypes.SELECT,
                        raw: true
                    ...
                    const itemList = await Item.sequelize.query(
                        "SELECT I.*, T.name as name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = I.id_item) as rating FROM items as I, types as T WHERE T.id_type = I.id_type AND I.status != 0 AND T.id_type = :id_type AND I.name COLLATE UTF8_GENERAL_CI LIKE :name ORDER BY I.price ASC", 
                    { 
                        replacements: { name: `%${name}%`, id_type: id_type },
                        type: QueryTypes.SELECT,
                        raw: true
                ...
                if(typesort == 1){
                    const itemList = await Item.sequelize.query(
                        "SELECT I.*, T.name as name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = I.id_item) as rating FROM items as I, types as T WHERE T.id_type = I.id_type AND I.status != 0 AND I.name COLLATE UTF8_GENERAL_CI LIKE :name ORDER BY rating DESC, I.price ASC", 
                    { 
                        replacements: { name: `%${name}%` },
                        type: QueryTypes.SELECT,
                        raw: true
                    ...
                    const itemList = await Item.sequelize.query(
                        "SELECT I.*, T.name as name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = I.id_item) as rating FROM items as I, types as T WHERE T.id_type = I.id_type AND I.status != 0 AND I.name COLLATE UTF8_GENERAL_CI LIKE :name ORDER BY I.price DESC", 
                    { 
                        replacements: { name: `%${name}%` },
                        type: QueryTypes.SELECT,
                        raw: true
                    ...
                    const itemList = await Item.sequelize.query(
                        "SELECT I.*, T.name as name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = I.id_item) as rating FROM items as I, types as T WHERE T.id_type = I.id_type AND I.status != 0 AND I.name COLLATE UTF8_GENERAL_CI LIKE :name ORDER BY I.price ASC", 
                    { 
                        replacements: { name: `%${name}%` },
                        type: QueryTypes.SELECT,
                        raw: true
            ...
            if(id_type){
                ...
                if(typesort == 1){
                    const itemList = await Item.sequelize.query(
                        "SELECT DISTINCT I.*, T.name as name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = I.id_item) as rating FROM items as I, types as T, reviews as R WHERE T.id_type = I.id_type AND I.status != 0 AND T.id_type = :id_type ORDER BY rating DESC, I.price ASC", 
                    { 
                    ...
                    const itemList = await Item.sequelize.query(
                        "SELECT DISTINCT I.*, T.name as name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = I.id_item) as rating FROM items as I, types as T, reviews as R WHERE T.id_type = I.id_type AND I.status != 0 AND T.id_type = :id_type ORDER BY I.price DESC", 
                    { 
                        replacements: { id_type: id_type },
                        type: QueryTypes.SELECT,
                        raw: true
                ...
                if(typesort == 1){//rating giam dan
                    const itemList = await Item.sequelize.query(
                        "SELECT I.*, T.name AS name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = I.id_item) as rating FROM items as I, types as T WHERE T.id_type = I.id_type AND I.status != 0 ORDER BY rating DESC, I.price ASC", 
                    {
                        type: QueryTypes.SELECT,
                        raw: true
                    });
                    ...
                    const itemList = await Item.sequelize.query(
                        "SELECT I.*, T.name AS name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = I.id_item) as rating FROM items as I, types as T WHERE T.id_type = I.id_type AND I.status != 0 ORDER BY I.price DESC", 
                    { 
                        type: QueryTypes.SELECT,
                        raw: true
                    });
...
const getDetailItem = async (req, res) => {
    ...
    try {
        const item = await Item.sequelize.query(
            "SELECT I.*, T.name as name_type, (SELECT ROUND(AVG(R.rating) * 2, 0) / 2 FROM reviews AS R WHERE R.id_item = :id_item) as rating, (SELECT COUNT(R.id_item) FROM reviews as R, items as I WHERE R.id_item = I.id_item AND I.id_item = :id_item) as countComment FROM items AS I, types as T WHERE T.id_type = I.id_type AND I.id_item = :id_item", 
        { 
            replacements: { id_item: id_item},
            type: QueryTypes.SELECT,
            raw: true
